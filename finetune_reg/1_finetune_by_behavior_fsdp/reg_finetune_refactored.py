...
def plot_loss(trainer,test_dataset, out_base, model_name, n_fold, i_fold, ds):
    ...
    e_x, e_y = series(f"eval_corr_mean")
    t_x, t_y = series(f"train_corr_mean")  # present only if LOG_TRAIN_CORR=1
    tex , tey = series(f"test_corr_mean")  # present only if LOG_TRAIN_CORR=1
...
def main():
    ...
    if HPO_TRIALS > 0:
        ...
        search_args = TrainingArguments(
            ...
            metric_for_best_model=f"eval_{global_eval_metric}_mean",
