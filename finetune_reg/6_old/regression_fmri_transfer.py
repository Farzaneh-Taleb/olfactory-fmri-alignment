...
def main():
    ...
    if not os.path.exists(f"{base_dir}/{out_dir}_fmri_metrics"):
        os.makedirs(f"{base_dir}/{out_dir}_fmri_metrics")
    ...
    for subject_dest in subjects_dest:
        ...
        for layer in range(layers_end[m],0,-1):
           ...
           if '_nc_' in out_dir:
            parent_input_sagar_original = f'{base_dir}/fmri/average_of_singletrial/fmri_{subject_dest}_{roi}.csv'
            df = pd.read_csv(parent_input_sagar_original)
            fmri_array =df_to_cid_voxel_array(df)
            nc_mask =create_nc_mask( base_dir,subject_dest,roi,threshold=0.25)
            remained_indices,fmri =mask_fmri(base_dir,fmri_array,nc_mask,subject_dest,roi)
            print("after masking fmri",fmri.shape,fmri_array.shape)
            if fmri.shape[1]==0:
                ...
                metrics.to_csv(f"{base_dir}/{out_dir}_fmri_metrics/all_laterfmrimetrics_{model_name}_{n_fold}_{num_train_epochs}_{subject_source}_{subject_dest}_{behavior_embeddings}_{unfreeze_last_n}_{layer}_{n_components}_{roi}.csv", index=False)

                continue
            else:
                fmri=read_orig_avg(base_dir, subject_dest, roi)
            ...
            for i_fold in range(n_fold):
                filename = (
                    f"finetuned_reg_fembeddings_transfer_{model_name}_{n_fold}_{num_train_epochs}_"
                ...
                embeddings = np.load(f"{base_dir}/read_orig_avg/finetuned_reg_fembeddings_transfer/{filename}")
                if 'zscored' in out_dir:
                    print("zscored")
                ...
                cids = pd.read_csv(f"{base_dir}/read_orig_avg/finetuned_reg_metrics/test_CIDs_{model_name}_{n_fold}_{num_train_epochs}_{subject_dest}_{behavior_embeddings}_{unfreeze_last_n}_{i_fold}.csv")
                ...
                indices_train = np.where(~np.isin(all_cids, cids_test.flatten()))[0]
