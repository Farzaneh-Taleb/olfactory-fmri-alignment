...
#SBATCH -t 24:00:00
#SBATCH -n 1
#SBATCH -p main,shared,memory,long
...
unfreeze_layers=(0)
z_scores=(True)  # <-- Now treating z_score as a grid with two options
...
total_combinations=$((num_folds * num_subjects * num_epochs * num_components * num_models * num_behaviors * num_unfreeze * num_zscores))
...
# Decode combination
fold_idx=$(( index / (num_subjects * num_epochs * num_components * num_models * num_behaviors * num_unfreeze * num_zscores) % num_folds ))
subj_idx=$(( index / (num_epochs * num_components * num_models * num_behaviors * num_unfreeze * num_zscores) % num_subjects ))
epoch_idx=$(( index / (num_components * num_models * num_behaviors * num_unfreeze * num_zscores) % num_epochs ))
ncomp_idx=$(( index / (num_models * num_behaviors * num_unfreeze * num_zscores) % num_components ))
model_idx=$(( index / (num_behaviors * num_unfreeze * num_zscores) % num_models ))
behavior_idx=$(( index / (num_unfreeze * num_zscores) % num_behaviors ))
unfreeze_idx=$(( index / num_zscores % num_unfreeze ))
...
$PYTHON_EXEC -V
$PYTHON_EXEC /cfs/klemming/projects/supr/olfactory_alignment/MoLFormer_fMRI/finetune_reg/5/regression_behavior_transfer.py \
  --subject "$subject" \
