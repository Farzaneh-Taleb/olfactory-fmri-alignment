...
def main():
    ...
    print("z_score",args.z_score)
    out_dir = args.out_dir
    z_score = args.z_score
    if not os.path.exists(f"{base_dir}/{out_dir}_metrics"):
        os.makedirs(f"{base_dir}/{out_dir}_metrics",exist_ok=True)
    pre='May15_'

    # choose all subjects except the one in args.subject
    ...
    for subject_dest in subjects_dest:
        for layer in range(1,layers_end[m]+1):
            # if type=='average_orig':
            behavior = np.load(f"{base_dir}/embeddings/embeddings_behavior_{subject_dest}_1.npy")
            ...
            if behavior_embeddings is not None:
                ...
                if behavior_embeddings.split(',') == ['']:
                    behavior_embeddings_int = int(behavior_embeddings)
                else:
                    behavior_embeddings_int = [
                    int(b.strip()) for b in behavior_embeddings.split(',')
                    ]
                behavior = behavior[:, behavior_embeddings_int]
            else:
                behavior_embeddings = ''  
            print(behavior.shape,flush=True)
            ...
            for i_fold in range(n_fold):
                ...
                print(f"reading {model_name}_{n_fold}_{num_train_epochs}_{subject_source}_{behavior_embeddings}_{unfreeze_last_n}_{i_fold}_{layer}.npy")
                ...
                smiles_df_dest = pd.read_csv(f"{base_dir}/embeddings/CIDs_smiles_selfies_{subject_dest}.csv")
                all_cids_dest = smiles_df_dest["CIDs"].values.tolist()
      
                cids = pd.read_csv(f"{base_dir}/read_orig_avg/{pre}finetuned_reg_metrics/test_CIDs_{model_name}_{n_fold}_{num_train_epochs}_{subject_dest}_{behavior_embeddings}_{unfreeze_last_n}_{i_fold}.csv")
                cids_dest_test = cids.values


                overlapping_indices,_ = find_overlap(base_dir,'',subject_dest)
                #find indices of cids_test in all_cids
                indices_test = np.where(np.isin(all_cids_dest, cids_dest_test.flatten()))[0]
                # Find indices of cids_test in all_cids
                ...
                behavior_train = behavior[indices_train]
                behavior_test = behavior[indices_test]
                embeddings_train.append(embedding_train)
                embeddings_test.append(embedding_test)
                behaviors_train.append(behavior_train)
                behaviors_test.append(behavior_test)
                # cids_dest_previousfold = [cid for cid in cids_dest_test_array if cid in overlapping_cids]
                



            # out_dir='finetuned_reg'
                # embeddings_random=np.random.rand(embeddings.shape[0],embeddings.shape[1])
            

            metrics = compute_correlation(embeddings_train,behaviors_train,embeddings_test,behaviors_test,None,n_components=n_components)
            metrics.to_csv(f"{base_dir}/{out_dir}_metrics/all_metrics_{model_name}_{n_fold}_{num_train_epochs}_{subject_source}_{subject_dest}_{behavior_embeddings}_{unfreeze_last_n}_{layer}_{n_components}_{z_score}.csv", index=False)
