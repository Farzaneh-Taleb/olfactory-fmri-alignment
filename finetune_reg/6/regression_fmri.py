    ...
    for subject_dest in subjects_dest:
        for layer in range(1,layers_end[m]+1):
            ...
            if read_style == 'avg_computed':
                ...
                parent_input_sagar_original = f'{base_dir}/fmri/average_of_singletrial_allTRs/fmri_{subject_dest}_{roi}.csv'

                df = pd.read_csv(parent_input_sagar_original)
                fmri =df_to_cid_voxel_array(df,tr)
                nc_mask_array =create_nc_mask( base_dir,subject_dest,roi,threshold=0.25)
                remained_indices,fmri =mask_fmri(base_dir,fmri,nc_mask_array,subject_dest,roi,nc_mask,func_mask)
