...
def main():
    ...
    for layer in range(1,layers_end[m]+1):
        ...
        for i_fold in range(n_fold):
            ...
            embeddings = np.load(f'{base_dir}/embeddings/embeddings_{model_name}_{subject}_{layer}.npy')
        ...
        if not os.path.exists(f"{base_dir}/{out_dir}_metrics"):
            os.makedirs(f"{base_dir}/{out_dir}_metrics")
        ...
        metrics.to_csv(f"{base_dir}/{out_dir}_metrics/all_frozenmetrics_{model_name}_{n_fold}_{subject}_{behavior_embeddings}_{layer}_{n_components}_{z_score}.csv", index=False)
