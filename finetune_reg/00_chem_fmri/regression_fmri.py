...
def main():
    ...
    print(args.nc_mask,args.func_mask,args.z_score)
    ...
    # for layer in range(layers_end[m],0,-1):
        ...
        if read_style == 'avg_computed':
            ...
            if fmri.shape[1]==0:
                ...
                metrics.to_csv(f"{base_dir}/read_orig_avg/{out_dir}_fmri_metrics/all_origfmrimetrics_{model_name}_{n_fold}_{subject}_{behavior_embeddings}_{layer}_{n_components}_{roi}_{nc_mask}_{func_mask}_{tr_orig}_{read_style}_{z_score}.csv", index=False)
                continue
        ...
        for i_fold in range(n_fold):
            ...
            behavior_train = fmri[indices_train]
            behavior_test = fmri[indices_test]
            embeddings_train.append(embedding_train)
            embeddings_test.append(embedding_test)
            fmris_train.append(behavior_train)
            fmris_test.append(behavior_test)


            
       
        metrics = compute_correlation(embeddings_train,fmris_train,embeddings_test,fmris_test,None,n_components=n_components)
        metrics.to_csv(f"{base_dir}/{out_dir}_fmri_metrics/all_origfmrimetrics_{model_name}_{n_fold}_{subject}_{behavior_embeddings}_{layer}_{n_components}_{roi}_{nc_mask}_{func_mask}_{tr_orig}_{read_style}_{z_score}.csv", index=False)
