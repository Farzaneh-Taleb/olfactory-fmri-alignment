...
parser.add_argument('--z_score', type=bool)
...
def main():
    ...
    for layer in range(1,layers_end[m]+1):
        ...
        behavior = np.load(f"{base_dir}/embeddings/embeddings_behavior_{subject}_1.npy")
        ...
        metrics.to_csv(f"{base_dir}/{out_dir}_metrics/all_laterfinetunedmetrics_{model_name}_{n_fold}_{num_train_epochs}_{subject}_{behavior_embeddings}_{unfreeze_last_n}_{layer}_{n_components}_{z_score}.csv", index=False)
